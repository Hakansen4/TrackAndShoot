using UnityEditor;
using UnityEngine;

// Original source: https://forum.unity.com/threads/scripts-useful-4-6-scripts-collection.264161/
// Modified.

namespace EasyClap.Seneca.Common.Editor
{
    public static class UGuiTools
    {
        private const string Menu = "Easy Clap/Utility/uGUI/";

        [MenuItem(Menu + "Anchors to Corners")]
        private static void AnchorsToCorners()
        {
            Undo.RecordObjects(Selection.transforms, Menu + "Anchors to Corners");

            foreach (var transform in Selection.transforms)
            {
                var t = transform as RectTransform;
                var pt = Selection.activeTransform.parent as RectTransform;

                if (t == null || pt == null) return;

                var rect = pt.rect;
                var newAnchorsMin = new Vector2(t.anchorMin.x + t.offsetMin.x / rect.width,
                    t.anchorMin.y + t.offsetMin.y / rect.height);
                var newAnchorsMax = new Vector2(t.anchorMax.x + t.offsetMax.x / rect.width,
                    t.anchorMax.y + t.offsetMax.y / rect.height);

                t.anchorMin = newAnchorsMin;
                t.anchorMax = newAnchorsMax;
                t.offsetMin = t.offsetMax = new Vector2(0, 0);

                PrefabUtility.RecordPrefabInstancePropertyModifications(t);
            }
        }

        [MenuItem(Menu + "Corners to Anchors")]
        private static void CornersToAnchors()
        {
            Undo.RecordObjects(Selection.transforms, Menu + "Corners to Anchors");

            foreach (var transform in Selection.transforms)
            {
                var t = transform as RectTransform;

                if (t == null) return;

                t.offsetMin = t.offsetMax = new Vector2(0, 0);

                PrefabUtility.RecordPrefabInstancePropertyModifications(t);
            }
        }

        [MenuItem(Menu + "Mirror Horizontally Around Anchors")]
        private static void MirrorHorizontallyAnchors()
        {
            Undo.RecordObjects(Selection.transforms, Menu + "Mirror Horizontally Around Anchors");
            MirrorHorizontally(false);
        }

        [MenuItem(Menu + "Mirror Horizontally Around Parent Center")]
        private static void MirrorHorizontallyParent()
        {
            Undo.RecordObjects(Selection.transforms, Menu + "Mirror Horizontally Around Parent Center");
            MirrorHorizontally(true);
        }

        [MenuItem(Menu + "Mirror Vertically Around Anchors")]
        private static void MirrorVerticallyAnchors()
        {
            Undo.RecordObjects(Selection.transforms, Menu + "Mirror Vertically Around Anchors");
            MirrorVertically(false);
        }

        [MenuItem(Menu + "Mirror Vertically Around Parent Center")]
        private static void MirrorVerticallyParent()
        {
            Undo.RecordObjects(Selection.transforms, Menu + "Mirror Vertically Around Parent Center");
            MirrorVertically(true);
        }

        private static void MirrorHorizontally(bool mirrorAnchors)
        {
            foreach (var transform in Selection.transforms)
            {
                var t = transform as RectTransform;
                var pt = Selection.activeTransform.parent as RectTransform;

                if (t == null || pt == null) return;

                if (mirrorAnchors)
                {
                    var oldAnchorMin = t.anchorMin;
                    t.anchorMin = new Vector2(1 - t.anchorMax.x, t.anchorMin.y);
                    t.anchorMax = new Vector2(1 - oldAnchorMin.x, t.anchorMax.y);
                }

                var oldOffsetMin = t.offsetMin;
                t.offsetMin = new Vector2(-t.offsetMax.x, t.offsetMin.y);
                t.offsetMax = new Vector2(-oldOffsetMin.x, t.offsetMax.y);

                t.localScale = new Vector3(-t.localScale.x, t.localScale.y, t.localScale.z);

                PrefabUtility.RecordPrefabInstancePropertyModifications(t);
            }
        }

        private static void MirrorVertically(bool mirrorAnchors)
        {
            foreach (var transform in Selection.transforms)
            {
                var t = transform as RectTransform;
                var pt = Selection.activeTransform.parent as RectTransform;

                if (t == null || pt == null) return;

                if (mirrorAnchors)
                {
                    var oldAnchorMin = t.anchorMin;
                    t.anchorMin = new Vector2(t.anchorMin.x, 1 - t.anchorMax.y);
                    t.anchorMax = new Vector2(t.anchorMax.x, 1 - oldAnchorMin.y);
                }

                var oldOffsetMin = t.offsetMin;
                t.offsetMin = new Vector2(t.offsetMin.x, -t.offsetMax.y);
                t.offsetMax = new Vector2(t.offsetMax.x, -oldOffsetMin.y);

                t.localScale = new Vector3(t.localScale.x, -t.localScale.y, t.localScale.z);

                PrefabUtility.RecordPrefabInstancePropertyModifications(t);
            }
        }
    }
}
